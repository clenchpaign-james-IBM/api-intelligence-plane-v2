"""
Index Schema 012: 5-Minute Metrics Index Template

Creates an index template for api-metrics-5m-* indices with monthly rotation.
Medium-resolution time-series metrics data for trend analysis and dashboards.
...
def create_metrics_5m_index_template(client: OpenSearch):
    """
    Create the api-metrics-5m-* index template with appropriate mappings.
    
    This template applies to:
    - api-metrics-5m-2026.03 (March 2026)
    - api-metrics-5m-2026.04 (April 2026)
    - etc.
    
    Stores:
    - 5-minute aggregated metrics from 1-minute buckets
    - Medium-resolution performance data for trend analysis
    ...
    Retention Policy:
    - Keep for 30 days for trend analysis and dashboards
    - Roll up to 1-hour buckets for longer retention
    """
    ...
    template_body = {
        ...
        "template": {
            "settings": {
                ...
                "refresh_interval": "30s",  # Slower refresh for aggregated data
                "index": {
...
def create_metrics_5m_ilm_policy(client: OpenSearch):
    """
    Create ILM (Index Lifecycle Management) policy for 5-minute metrics.
    
    Policy:
    - Hot phase: Keep for 7 days with standard refresh
    ...
    """
    policy_name = "api-metrics-5m-policy"
    ...
    policy_body = {
        "policy": {
            "description": "ILM policy for 5-minute metrics indices",
            "default_state": "hot",
            "states": [
                {
                    "name": "hot",
