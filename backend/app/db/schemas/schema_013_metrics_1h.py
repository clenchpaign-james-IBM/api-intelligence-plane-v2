"""
Index Schema 013: 1-Hour Metrics Index Template

Creates an index template for api-metrics-1h-* indices with monthly rotation.
Low-resolution time-series metrics data for historical analysis and reporting.
...
def create_metrics_1h_index_template(client: OpenSearch):
    """
    Create the api-metrics-1h-* index template with appropriate mappings.
    
    This template applies to:
    - api-metrics-1h-2026.03 (March 2026)
    - api-metrics-1h-2026.04 (April 2026)
    - etc.
    
    Stores:
    - 1-hour aggregated metrics from 5-minute buckets
    - Low-resolution performance data for historical analysis
    ...
    Retention Policy:
    - Keep for 90 days for historical analysis and reporting
    - Roll up to 1-day buckets for longer retention
    """
    ...
    template_body = {
        ...
        "template": {
            "settings": {
                ...
                "refresh_interval": "60s",  # Slow refresh for historical data
                "index": {
...
def create_metrics_1h_ilm_policy(client: OpenSearch):
    """
    Create ILM (Index Lifecycle Management) policy for 1-hour metrics.
    
    Policy:
    - Hot phase: Keep for 30 days with slow refresh
    - Warm phase: Move to warm nodes after 30 days
    ...
    """
    policy_name = "api-metrics-1h-policy"
    ...
    policy_body = {
        "policy": {
            "description": "ILM policy for 1-hour metrics indices",
            "default_state": "hot",
            "states": [
                {
                    "name": "hot",
