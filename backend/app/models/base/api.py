...
class IntelligenceMetadata(BaseModel):
    ...
    health_score: float = Field(
        ...
        description=(
            ...
            "Automatically calculated by the intelligence plane based on real-time metrics. "
            "\n\n"
            "**Calculation Method:**\n"
            "- Weighted average of four components:\n"
            "  - Error Rate (30%): 100 at 0% errors, 0 at 10%+ errors\n"
            "  - Response Time (25%): 100 at 0ms, 0 at 5000ms+\n"
            "  - Availability (25%): Direct metric value (0-100)\n"
            "  - Throughput Stability (20%): 100 at 0% CV, 0 at 100%+ CV\n"
            "- Based on metrics from last 24 hours (1-minute buckets)\n"
            "- Formula: (error_rate_score × 0.30) + (response_time_score × 0.25) + "
            "(availability_score × 0.25) + (throughput_score × 0.20)\n"
            "\n"
            "**Health Levels:**\n"
            "- 80-100: Healthy (optimal performance)\n"
            "- 50-79: Degraded (performance issues)\n"
            "- 30-49: Unhealthy (significant problems)\n"
            "- 0-29: Failing (critical issues)\n"
            "\n"
            "**Update Frequency:**\n"
            "- Computed every 5 minutes by scheduled job\n"
            "- Requires metrics data from last 24 hours\n"
            "- Skipped if no metrics available (keeps default)\n"
            "- Gateway-aware: processes APIs per gateway\n"
            "\n"
            "**Query Support:**\n"
            "- Natural language: 'healthy APIs', 'unhealthy', 'failing', 'degraded'\n"
            "- Direct field: intelligence_metadata.health_score\n"
            "- Operators: gte, lte, range\n"
            "- Examples:\n"
            "  - 'healthy' → health_score >= 80\n"
            "  - 'unhealthy' → health_score < 50\n"
            "  - 'failing' → health_score < 30\n"
            "  - 'degraded' → health_score >= 30 AND < 70\n"
            "\n"
            "**Related Fields:**\n"
            "- risk_score: Security risk assessment (separate concern)\n"
            "- security_score: Security posture (inverse of risk)\n"
            "- violation_count: Compliance violations (separate from health)\n"
            "\n"
            "**Implementation:**\n"
            "- Scheduler: backend/app/scheduler/intelligence_metadata_jobs.py::compute_health_scores_job()\n"
            "- Calculator: backend/app/scheduler/intelligence_metadata_jobs.py::calculate_health_score()\n"
            "- Component Calculators:\n"
            "  - calculate_error_rate_score(): Error rate analysis\n"
            "  - calculate_response_time_score(): Latency analysis\n"
            "  - calculate_availability_score(): Uptime analysis\n"
            "  - calculate_throughput_score(): Stability analysis (coefficient of variation)\n"
            "- Query Mapping: backend/app/services/query/concept_mapper.py (health concepts)\n"
            "\n"
            "**Example Values:**\n"
            "- 100.0: Perfect health (0% errors, fast response, 100% uptime, stable throughput)\n"
            "- 85.0: Healthy (minor issues, still performing well)\n"
            "- 65.0: Degraded (noticeable performance issues)\n"
            "- 45.0: Unhealthy (significant problems requiring attention)\n"
            "- 25.0: Failing (critical issues, immediate action needed)\n"
            "\n"
            "**Default Behavior:**\n"
            "- New APIs: 100.0 (optimistic default)\n"
            "- No metrics: Keeps previous score (not updated)\n"
            "- Insufficient data: Skipped until 24h of metrics available\n"
            "\n"
            "**Notes:**\n"
            "- This is a computed field, not set by gateway adapters\n"
            "- Reflects operational health, not security posture\n"
            "- Used for performance monitoring and alerting\n"
            "- Integrated with dashboard health indicators\n"
            "- Independent of security and compliance scores"
        ),
    )
    ...
    risk_score: Optional[float] = Field(
        ...
        le=100,
        description=(
            "Computed risk score (0-100, higher is worse). "
            "Automatically calculated by the intelligence plane based on security vulnerabilities. "
            "\n\n"
            "**Calculation Method:**\n"
            "- Weighted sum of vulnerabilities by severity:\n"
            "  - Critical: 25 points each\n"
            "  - High: 10 points each\n"
            "  - Medium: 3 points each\n"
            "  - Low: 1 point each\n"
            "- Capped at 100 (maximum risk)\n"
            "- 0 indicates no known vulnerabilities\n"
            "\n"
            "**Risk Levels:**\n"
            "- 0-24: Low risk\n"
            "- 25-49: Medium risk\n"
            ...
            "\n"
            "**Update Frequency:**\n"
            "- Computed every 10 minutes by scheduled job\n"
            "- Updated when vulnerabilities are added/removed\n"
            "- Used in security posture calculations\n"
            "\n"
            "**Query Support:**\n"
            "- Natural language: 'high risk APIs', 'low risk', 'risky'\n"
            "- Direct field: intelligence_metadata.risk_score\n"
            "- Operators: gte, lte, range\n"
            "\n"
            "**Related Fields:**\n"
            "- security_score: Inverse metric (100 = secure, 0 = vulnerable)\n"
            "- health_score: Overall API health (includes performance)\n"
            "- violation_count: Compliance violations (separate from security)\n"
            "\n"
            "**Implementation:**\n"
            "- Scheduler: backend/app/scheduler/intelligence_metadata_jobs.py::compute_risk_scores_job()\n"
            "- Calculator: backend/app/scheduler/intelligence_metadata_jobs.py::calculate_risk_score()\n"
            "- Security Service: backend/app/services/security_service.py::_calculate_risk_score()\n"
            "- Query Mapping: backend/app/services/query/concept_mapper.py (risk concepts)\n"
            "\n"
            "**Example Values:**\n"
            "- 0.0: No vulnerabilities detected\n"
            "- 15.0: 1 high + 5 low vulnerabilities\n"
            "- 75.0: 3 critical vulnerabilities (triggers critical alert)\n"
            "- 100.0: 4+ critical vulnerabilities (maximum risk)\n"
            "\n"
            "**Shadow APIs:**\n"
            "- Default risk_score: 75.0 (undocumented = high risk)\n"
            "- Automatically flagged for security review\n"
            "\n"
            "**Notes:**\n"
            "- This is a computed field, not set by gateway adapters\n"
            "- None indicates score not yet calculated\n"
            "- Used for prioritizing security remediation efforts\n"
            "- Integrated with security posture dashboard\n"
            "- Triggers alerts when >= 75 (critical threshold)"
        ),
    )
    ...
    violation_count: int = Field(
        default=0, ge=0, description="Total number of compliance violations for this API"
    )
